...
// Dynamic CORS configuration to support credentials with any origin in development
const corsConfig = {
    origin: (origin: string | undefined, callback: (err: Error | null, origin?: string | boolean) => void) => {
        // Allow requests with no origin (like mobile apps or curl)
        if (!origin) return callback(null, true);
        
        // In development or when CORS_ORIGIN is '*', reflect the requesting origin
...
server.listen(port, () => {
    console.log(`RoyalChess API server listening on :${port}`);
});
